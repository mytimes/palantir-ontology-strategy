...
- **是否添加原始索引？**
  ...
  - 由于这个额外索引，对于应用了渲染提示的属性，编入 Object Storage v1（Phonograph）的列总数中会包含两列。
...
| Searchable（可搜索） | - 如果该属性不会在应用中被搜索或排序，**禁用**可提升重新索引性能。 - 如果该属性包含大型字符串，性能提升将尤为显著。- 必须选中 Searchable，应用才会采用 Selectable、Sortable 或 Low cardinality 渲染提示。 | 是 | 是 |
