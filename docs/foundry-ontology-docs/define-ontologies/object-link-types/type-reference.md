...
**对象类型**是对现实世界实体或事件的模式定义，由各个对象组成。例如，`JFK` 和 `LHR` 都可以是 `Airport`（机场）对象类型的对象。
...
对象类型定义（有时简称为“对象类型”）指关于本体实体（如对象类型、链接类型和动作类型）的类型级信息。例如，对象类型的元数据可能包括显示名、属性名称、属性数据类型和描述。元数据并不涉及对象类型属性或主键的实际数据或值；这些被视为本体数据。
