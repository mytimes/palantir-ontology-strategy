...
结构体属性可以被本地属性类型和共享属性类型使用。在将本地属性类型转换（或提升）为共享属性类型时，需要重新映射结构体字段。由共享属性类型支撑的本地结构体属性类型将继承共享属性类型的字段，但结构体字段资源标识符（RID）除外。结构体字段元数据（显示名、描述、别名）则会从共享属性类型继承，但结构体字段将保留其原始 RID。
