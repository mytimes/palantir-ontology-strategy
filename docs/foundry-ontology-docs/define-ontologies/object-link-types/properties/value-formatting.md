...
| **最大小数位数** | 小数点后显示的最多位数。小数位数更多的值将四舍五入到此长度。 | 设置为 `2` 可将 `3.14159` 显示为 `3.14`。 |
| **最小小数位数** | 小数点后显示的最少位数。较短的值将用末尾零填充。 | 设置为 `2` 可将 `3.5` 显示为 `3.50`。 |
| **最大/最小有效数字位数** | 显示的最多和最少有效数字位数，从第一个非零数字开始计数。 | 将最大有效数字位数设置为 `3` 可将 `3.14159` 显示为 `3.14`。 |
| **最小整数位数** | 小数点前显示的最少位数，用前导零填充。 | 设置为 `2` 可将 `5` 显示为 `05`。 |
| **预览结果** | 查看和测试数字格式设置。 | 在输入框中输入任何与你期望在属性值中看到的类似的数字，即可预览格式设置效果。 |
...
值格式设置可应用于作为 Foundry/Multipass 用户 ID 或用户组 ID 的字符串，通过选择 **Username or group name**（用户名或用户组名称）选项，将它们转换为显示用户的名字和姓氏或用户组名称。此值格式设置选项通常用于以下情况：你创建了一个[动作](https://www.palantir.com/docs/foundry/action-types/overview/)来编辑属性，并在其中一个属性字段中存储用户 ID 或用户组 ID。在支撑数据中，此信息将存储为用户的 Foundry 用户 ID 或用户组 ID，而应用值格式设置可以渲染用户名或用户组名称而不是 ID。
