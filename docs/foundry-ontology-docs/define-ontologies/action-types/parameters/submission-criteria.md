...
Multipass 属性被视为字符串列表，只能与其他字符串或字符串列表进行比较。用户将拥有一份其有权访问的建议 Multipass 属性列表。使用 `Other user attribute` 字段，可以针对用户无权访问的属性配置条件。如果用户无权访问某个属性，则该条件将失败。
...
选择条件模板后，选择要比较的值。某些参数（如列表参数或对象参数）需要对比较中应使用的值进行更细粒度的选择。我们也可以选择比较列表的长度而非其内容。
...
> **ℹ️ 提示：示例**
>
> 由于在我们的示例中用户是多个用户组的成员，而比较对象是单个用户组，因此我们需要选择 `includes` 运算符来检查是否有重叠。然而，运营状态需要与预期状态完全匹配，因此必须设置 `is` 运算符。
