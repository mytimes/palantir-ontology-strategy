...
对象下拉菜单校验中的静态值筛选器会暴露给所有能够查看该动作类型的用户。使用这些筛选器有可能向无权查看被筛选对象的用户暴露属性值组合。依靠对象属性或参数来筛选对象集可以降低这一风险。这些值不会直接显示在界面中。
...
- `Investigation Object` 参数将遵循该用户现有的对象可见性限制。
...
这意味着这些值在界面中对任何用户都**不可见**。如果可见性比安全性更受关注，则可以忽略此警告。
