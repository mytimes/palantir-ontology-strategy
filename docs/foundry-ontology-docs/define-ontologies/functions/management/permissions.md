...
当函数加载对象数据时（无论是作为参数还是通过[对象搜索](https://www.palantir.com/docs/foundry/functions/api-object-sets/)），运行函数的最终用户的权限决定了加载哪些对象。对于使用行级权限保护的对象类型，这意味着执行同一函数的不同用户可能得到不同结果。此行为是设计使然——用户只应看到他们有权访问的对象，该行为使单个函数能够适用于对各个对象具有不同访问权限的用户。
