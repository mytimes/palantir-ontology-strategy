...
**Extended function execution（扩展函数执行）** 模态框允许特定函数在启用某些扩展能力的情况下执行：
...
由于这些能力会授予函数更高的访问权限，扩展函数执行能力通过模态框可折叠部分中配置的一组允许列表，细粒度控制这些能力在何处启用。函数必须先满足相关允许列表的要求，才能以这些扩展能力发布、执行或安装。配置以下允许列表，以控制哪些仓库、函数和项目可以使用扩展函数执行。
